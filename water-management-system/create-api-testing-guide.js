 ...
 * 
 * This script provides example curl commands and test cases
 * for verifying the entire hardware integration pipeline
 */
...
3. Set URL: \`https://your-project-ref.functions.supabase.co/functions/v1/ingest-reading\`
4. Headers tab:
   - \`x-device-key\`: \`WM-0001:your-device-secret\`
...
\`\`\`
6. Click Send
7. Should see 200 response with \`"ok": true\`
...
\`\`\`
**Cause:** serial_number in URL doesn't match payload
...
fs.writeFileSync(
  ...
  (err) => {
    ...
    else console.log("✅ Created API_TESTING_GUIDE.md");
  }
);
